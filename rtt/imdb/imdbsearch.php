<?php
 ...
 #############################################################################
 # $Id: imdbsearch.php 253 2009-10-11 21:54:15Z izzy $
...
echo "<TABLE ALIGN='center' BORDER='1' STYLE='border-collapse:collapse;margin-top:20px;'>\n"
   . " <TR><TH STYLE='background-color:#ffb000'>Select a $headname to Insert</TH></TR>";//<TH STYLE='background-color:#ffb000'>IMDB page</TH></TR>";
foreach ($results as $res) {
  switch($_GET["searchtype"]) {
    case "nm" :
      ...
      break;
    default   :
      echo " <TR><TD><center><a href='../processform.php?mid=".$res->imdbid()."&class=".$class."&ObjectId=".$ObjectId."'>".$res->title()." (".$res->year().")</a></center></TD>"
